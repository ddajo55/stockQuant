...
def clean_fs(df, ticker, frequency):
    # '계정'컬럼 이외의 value에 대해 Nan을 포함한 행을 추출하여 all함수를 이용해 행 삭제
    df = df[~df.loc[:, ~df.columns.isin(['계정'])].isna().all(axis=1)]
    #'계정' 컬럼에서 중복된 행들 중에 첫번째 값을 제외한 나머지 데이터 삭제
    df = df.drop_duplicates(['계정'], keep='first')
    # 여러 행들의 값들을 melt함수를 통해 하나의 행으로 합치기
    df = pd.melt(df, id_vars='계정', var_name='기준일', value_name='값')
    # null(None)값 삭제
    df = df[~pd.isnull(df['값'])]
    # 계정 컬럼에서 '계산에 참여한 계정 펼치기 : ' value를 가진 행 삭제
    df['계정'] = df['계정'].replace({'계산에 참여한 계정 펼치기': ''}, regex=True)
    # 기준일 column에 들어갈 날짜값 세팅
...
for i in tqdm(range(0, len(ticker_list))):
    ...
    try:
        ...
        # 결산년에 해당하는 계정만 남기기
        data_fs_y = data_fs_y.loc[:, data_fs_y.columns.isin(['계정']) | (data_fs_y.columns.str[-2:].isin(fiscal_data_text))]
